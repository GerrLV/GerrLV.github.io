...
conn =pymysql.connect(host='localhost',user='root',password='Gr4e1Y00',database='Asistencia')
cursor=conn.cursor()

cursor.execute("Select nombre,matricula,grupo from lista")
...
for nombre, matricula, grupo in data:
    fecha = ''  # or set e.g. fecha = datetime.date.today().isoformat()
    qr = qrcode.QRCode(
        version=1,
        box_size=10,
        border=5
    )
    info = f'Matricula: {matricula}\nNombre: {nombre}\nGrupo: {grupo}\nFecha: {fecha}'
    qr.add_data(info)
    qr.make(fit=True)
    img = qr.make_image(fill_color='black', back_color='white')
    img.save(f'qr_{matricula}.png')